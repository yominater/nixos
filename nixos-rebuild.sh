...
#
# Immediatly exit the script if a command returns anything other than 0
set -e

# Edit your config
sudo -E $EDITOR /etc/nixos/configuration.nix
...
current=$(nixos-rebuild list-generations | grep True | awk '{print $1, $4}')

# Commit all changes witih the generation metadata
sudo -E git commit -am "$current"
